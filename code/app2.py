...
def find():
    ...
    #
    binance.get_historical_klines(paridad, intervalo, comienzo)
    binance.get_resultado(12, 17, 26, 14)
    graficos = gm(binance.get_resultado(12, 17, 26, 14),
                  paridad, intervalo, comienzo)
...
def gm(data, paridad, intervalo, comienzo):
    ...
    fig = go.Figure(data=[go.Scatter(x=df.index, y=df['Cierre'], mode='lines', name='Cierre', line=dict(color='blue', width=2)),
                          go.Scatter(x=df.index, y=df['SMASL'], mode='lines', name='SMASL', line=dict(
                              color='red', dash='dash')),
                          go.Scatter(x=df.index, y=df['SMAME'], mode='lines', name='SMAME', line=dict(
    ...
    fig2 = go.Figure(data=[go.Scatter(x=df.index, y=df['RSI'], mode='lines', name='RSI', line=dict(color='blue')),
